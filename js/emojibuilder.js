...
// The actual location of all the SVG's is stored in /svgs_MODE.json, which is generated by /fetcher/svg.php.
async function getSVGFilesInDirectory(mode) {
  let directory, json;
  
  switch (mode) {
    case MODES.DETAILED: 
      directory = URLS.SVG_DIR_PATH_DETAILED;
      json = URLS.JSON_PATH_DETAILED;
      break;
    case MODES.LATEST:
      directory = URLS.SVG_DIR_PATH_LATEST;
      json = URLS.JSON_PATH_LATEST;
      break;
    default:
      directory = URLS.SVG_DIR_PATH_DETAILED;
      json = URLS.JSON_PATH_DETAILED;
